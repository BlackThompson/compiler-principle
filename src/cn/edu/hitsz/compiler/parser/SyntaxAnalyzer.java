...
public class SyntaxAnalyzer {
    ...
    public void run() {
        ...
        // create token stack and status stack
        Stack<Status> statusStack = new Stack<>();
        Stack<Symbol> symbolStack = new Stack<>();

        // initialise token stack and status stack
        symbolStack.push(new Symbol(Token.simple("$")));
...
//                    String str_production = production.toString();
//                    String[] contains = str_production.split(" ");
//                    NonTerminal leftPart = new NonTerminal(contains[0]);

                    NonTerminal leftPart = production.head();
                    ...
                    Status nextStatus = lrTable.getGoto(statusStack.peek(), leftPart);
                    statusStack.push(nextStatus);
                    String str = leftPart.toString();
                    symbolStack.push(new Symbol(leftPart));
                    // callWhenInReduce(statusStack.peek(), production);
                }
                ...
                case Error -> {
                    throw (new RuntimeException("SyntaxAnalysis Error"));
                }
